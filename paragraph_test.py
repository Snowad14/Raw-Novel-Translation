...
async def infer(
	...
	if detector == 'default' :
		...
		for region in text_regions :
			print(f"DEBUG --> {region.text} \n\n")
			for idx in region.textline_indices :
				txtln = textlines[idx]
				#print(f"DEBUG --> {region.text} \n\n")
				#textPositions.append((region.text, txtln.pts[0][0]))
	...
	# sortedTextPositions = list(set(sorted(textPositions, key=lambda tup: tup[1], reverse=True)))
